...
const ARDaCSurvivalCurve = ({
  fetchAndUpdateRawData, casecount, guppyConfig, filter,
}) => {
  ...
  const getGroupingFields = () => {
    ...
    if (studyNameFilter) {
      ...
      if (Array.isArray(studyNameFilter)) {
        ...
        isClinical = filterValues.some((val) => (Array.isArray(val) ? val.includes('clinical_trial') : val === 'clinical_trial'),
        );
        isObservational = filterValues.some((val) => (Array.isArray(val) ? val.includes('observational') : val === 'observational'),
        );
      }
  ...
  const checkDataQuality = (data, outcomeType) => {
    ...
    data.forEach((patient) => {
      ...
      if (outcomeType === 'death') {
        if (patient.vital_status === 'dead') {
          ...
          quality.exclusionReasons.invalidVitalStatus++;
          excluded = true;
        }
    ...
    if (quality.exclusionReasons.deathNoValidTime > 0) {
      quality.warnings.push(`${quality.exclusionReasons.deathNoValidTime} patients excluded: invalid or missing days_to_death (≤0 or blank)`);
    }
    if (quality.exclusionReasons.akiNoValidTime > 0) {
      quality.warnings.push(`${quality.exclusionReasons.akiNoValidTime} patients excluded: invalid or missing days_to_aki (≤0 or blank)`);
    }
    ...
    if (quality.exclusionReasons.invalidVitalStatus > 0) {
      quality.warnings.push(`${quality.exclusionReasons.invalidVitalStatus} patients excluded: invalid vital_status`);
    }
  ...
  const prepareSurvivalData = (rawData, outcome, groupBy) => {
    ...
    const getGroupValue = (patient, groupBy) => {
      ...
      if (groupBy === 'actarm') {
        return patient.actarm || patient.treatment_arm || patient.act_arm || 'OBS patients';
      }
      if (groupBy === 'cohort') {
        return patient[groupBy] || 'RCT patients';
      }
      return patient[groupBy] || 'Other';
    };
  ...
  const calculateSurvivalData = (patients) => {
    ...
    for (const time of Object.keys(timeGroups).sort((a, b) => a - b)) {
      ...
      // after events add censored points
      if (censored > 0 && parseInt(time) <= 180) {
        censoredPoints.push({
          time: parseInt(time),
          survival: survivalProb, // use survivalProb
        });
      }
...
`;
  ...
  const CHART_CONFIG = {
    ...
    height: 400,
    margin: {
      top: 20, right: 30, left: 70, bottom: 70,
    },
    dataWidth: 700,
    ...
    <div className='w-full p-4'>
      ...
      <div className='flex justify-between items-center mb-4'>
        ...
        <div className='flex gap-4'>
          <div className='flex flex-col'>
            <label style={{
              fontSize: '0.875rem', fontWeight: '500', marginBottom: '8px', marginRight: '10px',
            }}
            >
              Outcome
            </label>
      ...
      {dataQuality.warnings && dataQuality.warnings.length > 0 && (
        ...
        >
          ...
          <div style={{ position: 'relative' }} className='info-icon-container'>
            ...
            >
              <div style={{
                fontWeight: '600', marginBottom: '8px', fontSize: '0.875rem', color: '#1F2937',
              }}
              >
                Data Quality Warnings:
              </div>
              <ul style={{
                fontSize: '0.8rem', margin: 0, paddingLeft: '1.2rem', lineHeight: '1.5',
              }}
              >
      ...
      >
        ...
        <div style={{ textAlign: 'center', marginBottom: '16px' }}>
          <h4 style={{
            fontSize: '1rem', fontWeight: '600', color: '#374151', margin: 0,
          }}
          >
