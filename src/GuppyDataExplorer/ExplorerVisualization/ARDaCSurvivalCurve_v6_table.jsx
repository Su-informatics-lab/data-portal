...
const ARDaCSurvivalCurve = ({ fetchAndUpdateRawData, casecount, guppyConfig, filter }) => {
  ...
  useEffect(() => {
    ...
    const calculateRiskTable = (survivalData) => {
      ...
      return { riskTimePoints, riskTable };
    };

    const { riskTimePoints, riskTable } = calculateRiskTable(groupedData);
    setRiskTableData({ riskTimePoints, riskTable });
...
`;
  ...
  setTimeout(() => {
    ...
    console.log('图表左边距:', chartRect.left);
    console.log('X轴刻度真实位置:');
    
    ticks.forEach((tick, index) => {
      const tickRect = tick.getBoundingClientRect();
      const relativeX = tickRect.left - chartRect.left + (tickRect.width / 2);
      console.log(`刻度${index}: ${relativeX}px (文字: ${tick.textContent})`);
    });
  }, 1000);
  ...
  console.log('X轴刻度坐标：', {
    ...
    '60天': getXPosition(60),
    '90天': getXPosition(90),
    '120天': getXPosition(120),
    '150天': getXPosition(150),
    '180天': getXPosition(180)
  });
            ...
            {Object.entries(riskTableData.riskTable).map(([group, groupRiskData], groupIndex) => (
              <div key={group} style={{ 
                ...
                <div style={{ 
                  ...
                  fontWeight: '500',
                  color: COLORS[groupIndex % COLORS.length],
                  textAlign: 'right',                          // ← 从'left'改为'right'
                  whiteSpace: 'nowrap',
                  overflow: 'visible',                         
                  position: 'relative',
                  left: '-60px'
                ...
                {CHART_CONFIG.timePoints.map(timePoint => (
                  <div 
                    ...
                    style={{ 
                      position: 'absolute',
                      left: `${getXPosition(timePoint)}px`,      // 精确的X轴坐标
                      transform: 'translateX(-50%)',             // 自动居中，不管内容宽度
                      fontSize: '0.85rem',
                      color: '#374151',
                      textAlign: 'center'
                      // 不设置width，让内容自适应
