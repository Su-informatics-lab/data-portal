...
class ExplorerFilter extends React.Component {
  ...
  // 添加这两个辅助函数
  renameNoDataInAllFields = (aggsData) => {
    if (!aggsData) return aggsData;
    
    // 定义字段特定的映射
    const fieldMapping = {
      'actarm': 'Cases in OBS(steroid)',
      'cohort': 'AH cases in RCT',
      'case_arm': 'Cases in OBS(steroid)',
      'case_group': 'AH cases in RCT',
    };
    
    const newAggsData = Object.keys(aggsData).reduce((res, field) => {
      const fieldData = aggsData[field];
      if (fieldData && fieldData.histogram) {
        const newHistogram = fieldData.histogram.map(item => {
          let newKey = item.key;
          if (item.key === 'no data') {
            newKey = fieldMapping[field] || 'Unknown';
          }
          return {
            ...item,
            key: newKey
        ...
        res[field] = { ...fieldData, histogram: newHistogram };
      } else {
        res[field] = fieldData;
      }
      return res;
  ...
  getOriginalKey = (currentKey, field) => {
    // 反向映射，用于检查权限
    const fieldMapping = {
      'actarm': 'Unknown Treatment',
      'race': 'Unknown Race', 
      'gender': 'Unknown Gender',
      'study_site': 'Unknown Site',
      'laboratory_test': 'No Test Data',
      'cohort': 'Unknown Group',
      'ethnicity': 'Unknown Ethnicity',
      'case_arm': 'Unknown Arm',
      'case_group': 'Unknown Group',
      'specimen_type': 'Unknown Type',
      'PI_name': 'Unknown Lab',
    ...
    if (currentKey === fieldMapping[field] || currentKey === 'Unknown') {
      return 'no data';
    }
    return currentKey;
  };
  ...
  onProcessFilterAggsData = (aggsData) => {
    // 先处理重命名逻辑，无论什么 tierAccessLevel
    const processedAggsData = this.renameNoDataInAllFields(aggsData);
    
    if (this.props.tierAccessLevel !== 'regular') {
      return processedAggsData;
    }
    if (processedAggsData === null) {
      return processedAggsData;
    }
    const newAggsData = Object.keys(processedAggsData).reduce((res, field) => {
      ...
      if (!Object.keys(this.props.accessibleFieldObject).includes(field)) {
        res[field] = processedAggsData[field];
        return res;
      }
      ...
      const newHistogram = processedAggsData[field].histogram
        .filter(({ key }) => {
          // 注意：这里需要检查原始的 "no data" 值，因为我们已经重命名了
          const originalKey = this.getOriginalKey(key, field);
          const accessible = accessibleValues.includes(originalKey);
        ...
        .map(({ key, count }) => {
          ...
          return {
            key,
            count,
            accessible: accessibleValues.includes(originalKey),
          };
        });
